...
mod test {
    ...
    fn charactor() {
        ...
        let (_, parsed) = syntax::CharLiteralParser::new().parse("'\\\\'").unwrap();
        assert!(parsed == AstLiteral_::Char(b'\\'));
        let (_, parsed) = syntax::CharLiteralParser::new().parse("'\\0'").unwrap();
    ...
    #[test]
    fn float() {
        let (_, parsed) = syntax::FloatLiteralParser::new().parse("0.0").unwrap();
        assert!(parsed == AstLiteral_::Float(0.0));
        let (_, parsed) = syntax::FloatLiteralParser::new().parse("1.0").unwrap();
        assert!(parsed == AstLiteral_::Float(1.0));
        let (_, parsed) = syntax::FloatLiteralParser::new().parse("123.0").unwrap();
        assert!(parsed == AstLiteral_::Float(123.0));
        let (_, parsed) = syntax::FloatLiteralParser::new().parse("0.123").unwrap();
        assert!(parsed == AstLiteral_::Float(0.123));
        let (_, parsed) = syntax::FloatLiteralParser::new().parse("0.123e10").unwrap();
        assert!(parsed == AstLiteral_::Float(0.123e10));
        let (_, parsed) = syntax::FloatLiteralParser::new()
            .parse("0.123e-10")
            .unwrap();
        ...
        let (_, parsed) = syntax::FloatLiteralParser::new().parse("-1.0").unwrap();
        assert!(parsed == AstLiteral_::Float(-1.0));
        let (_, parsed) = syntax::FloatLiteralParser::new().parse("1.0e10").unwrap();
        assert!(parsed == AstLiteral_::Float(1.0e10));
    }

    #[test]
    fn string() {
        let (_, parsed) = syntax::StringLiteralParser::new()
            .parse(r###""""###)
            .unwrap();
    ...
    #[test]
    fn array() {
        let (_, parsed) = syntax::LiteralParser::new().parse(r##"{}"##).unwrap();
        assert!(parsed == AstLiteral_::List(vec![]));
        let (_, parsed) = syntax::LiteralParser::new().parse(r##"{1}"##).unwrap();
        match parsed {
        ...
        let (_, parsed) = syntax::LiteralParser::new().parse(r##"{1, 2}"##).unwrap();
        match parsed {
            AstLiteral_::List(list) => {
                assert!(list[0].1 == AstLiteral_::Int(1));
        ...
        let (_, parsed) = syntax::LiteralParser::new()
            .parse(r##"{1, 2, 3, }"##)
            .unwrap();
        match parsed {
            AstLiteral_::List(list) => {
